...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderService : IOrderService
    {
        ...
        private readonly IRepository<DeliveryManOrderMapping> _deliveryManOrderMapingRepository;
        ...
        public OrderService(IHtmlFormatter htmlFormatter,
            ...
            IRepository<DeliveryManOrderMapping> deliveryManOrderMapingRepository,
            ...
            )
        {
            ...
            _deliveryManOrderMapingRepository = deliveryManOrderMapingRepository;
        ...
        public virtual async Task<IPagedList<Order>> SearchOrdersAsync(int storeId = 0,
            ...
            string orderNotes = null, int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false, int deliveryManId = 0, bool deliveryManCompletedOrders = false)
        {
            ...
            //bhnau chauhan 09/06/2022
            if (deliveryManId > 0)
            {
                query = from o in query
                        join oi in _orderItemRepository.Table on o.Id equals oi.OrderId
                        join p in _deliveryManOrderMapingRepository.Table on o.Id equals p.OrderId
                        where p.DeliveryManId == deliveryManId && p.OrderStatus == true
                        select o;

                    //for showing order list with delivery man order along with other order which has not accepted yet
                    query = query.Union(from o in _orderRepository.Table
                                        where !_deliveryManOrderMapingRepository.Table.Any(f => f.OrderId == o.Id)
                                        select o);
                    ...
                    query = query.Union(from o in _orderRepository.Table
                                        where _deliveryManOrderMapingRepository.Table.Any(f => f.OrderId == o.Id && f.DeliveryManId != deliveryManId
                                        && f.OrderStatus == false)
                                        select o);



                    var isMappingexists = _deliveryManOrderMapingRepository.Table.Where(x => x.DeliveryManId == deliveryManId && x.OrderStatus == false);
                    var mappings = isMappingexists.Distinct().ToList();
                    if (mappings != null)
                    {
                        query = query.Where(x => !mappings.Any(y => y.OrderId == x.Id));
                    }
                    query = query.Distinct();
            }

            if (deliveryManCompletedOrders)
            {
                query = (from o in _orderRepository.Table
                         where _deliveryManOrderMapingRepository.Table.Any(f => f.OrderId == o.Id)
                         select o);
                query = query.Where(x => x.OrderStatusId == (int)OrderStatus.Complete && x.ApproveDistanceinKms);
            }
        ...
        public virtual async Task<OrderItem> GetAllOrders(int orderId)
        {




            return  await( from o in _orderItemRepository.Table
                       join v in _vendorRepository.Table
                       
                       on o.VendorId equals v.Id

                       join d in _deliveryManOrderMapingRepository.Table
                       on o.OrderId equals d.OrderId
                       join c in _customerRepository.Table                     

                       on d.DeliveryManId equals c.Id
