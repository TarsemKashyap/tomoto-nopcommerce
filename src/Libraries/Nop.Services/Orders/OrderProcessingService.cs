...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
            ...
            ProcessPaymentResult processPaymentResult, PlaceOrderContainer details)
        {
            ...
            var deliverLat = _httpContextAccessor.HttpContext.Session.GetString("DeliveryLat");
            var deliveryLng = _httpContextAccessor.HttpContext.Session.GetString("DeliveryLng");

            decimal lat;
            decimal lng;
            Decimal.TryParse(deliverLat, out lat);
            Decimal.TryParse(deliveryLng, out lng);
        ...
        protected virtual async Task SendNotificationsAndSaveNotesAsync(Order order)
        {
            ...
            //bhanu chauhan 09/06/2022
            // order placed notification for delivery Man
            CustomerRole customerRole = await _customerService.GetCustomerRoleBySystemNameAsync(NopCustomerDefaults.DeliveryManRoleName);
            IList<Customer> deliveryManList = await _customerService.GetCustomerbyCustomerRole(customerRole.Id);
            foreach (var deliveryMan in deliveryManList)
            {
                var ids = await _workflowMessageService.SendOrderPlacedDeliveryManNotificationAsync(order, deliveryMan, _localizationSettings.DefaultAdminLanguageId);
                if (ids.Any())
                    await AddOrderNoteAsync(order, $"\"Order placed\" email (to Delivery Man) has been queued. Queued email identifiers: {string.Join(", ", ids)}.");
            }
