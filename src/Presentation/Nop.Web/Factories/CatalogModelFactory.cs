...
namespace Nop.Web.Factories
{
    public partial class CatalogModelFactory : ICatalogModelFactory
    {
        ...
        protected virtual async Task PrepareCatalogProductsAsync(CatalogProductsModel model, IPagedList<Product> products, bool isFiltering = false, List<LatLangModel> latLangModel = null)
        {
            ...
            {
                model.Products = (await _productModelFactory.PrepareProductOverviewModelsAsync(products,true,true,null,false,false,latLangModel)).ToList();
        ...
        /// </returns>
        public virtual async Task<CategoryModel> PrepareCategoryModelAsync(Category category, CatalogProductsCommand command,List<LatLangModel> latLangModel = null)
        {
            if (category == null)
            ...
            var model = new CategoryModel
            {
                ...
                CatalogProductsModel = await PrepareCategoryProductsModelAsync(category, command,latLangModel)
            };
        ...
        /// </returns>
        public virtual async Task<CatalogProductsModel> PrepareCategoryProductsModelAsync(Category category, CatalogProductsCommand command, List<LatLangModel> latLangModel = null)
        {
            if (category == null)
                throw new ArgumentNullException(nameof(category));
            ...
            if (latLangModel != null)
            {
                 result = latLangModel.Select(x=>x.vendorId).Distinct().ToList();
            }
            //products
            var products = await _productService.SearchProductsAsync(
                ...
                orderBy: (ProductSortingEnum)command.OrderBy,vendorIds:result);

            var isFiltering = filterableOptions.Any() || selectedPriceRange?.From is not null;
            await PrepareCatalogProductsAsync(model, products, isFiltering,latLangModel);
